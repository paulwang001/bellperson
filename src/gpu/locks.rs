...
pub fn prove_mode()-> String{
    if let Ok(mut f) = std::fs::OpenOptions::new().read(true).open(tmp_path("mode")){
        ...
        if let Ok(_s) = f.read_to_string(&mut mode){
            return mode;
        }
    }
...
pub fn get_all_device_and_lock(count:u8,retry:u32)->Vec<u32> {
    ...
    let unlocked = {
        let lockable_device = LOCKABLE_DEVICES.clone();
        let lockable_device = lockable_device.lock().unwrap();
        lockable_device.iter().filter(|x|x.1 == 0).map(|x|x.0).collect::<Vec<u32>>()
    };
    let mut v_idx = vec![];
    if !unlocked.is_empty() {
        let lockable_device = LOCKABLE_DEVICES.clone();
        let lockable = lockable_device.lock().unwrap();
...
impl GPULock {
    ...
    pub fn lock_count_default(name:&str,count:u8) -> GPULock {
        ...
        let size = match std::env::var(format!("FIL_PROOFS_LOCK_{}",name.to_uppercase())){
